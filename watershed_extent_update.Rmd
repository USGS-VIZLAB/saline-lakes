...
##### **Steps to creating watershed extent**
  ...
  3. Created a spreadsheet that maps all the HUC6, HUC8,and HUC10 associated with each of our lakes (see HUC to Lake sharepoint excel [here](https://doimspp.sharepoint.com/:x:/s/IIDDStaff/EdhaQ9_tsPNIhW32hI9nydAB9RO5CDXSHx3yGLVWtaxwNA?e=wl8wS4)). We then narrowed the scope of the extent following the feedback and consultation with experts. We identified the HUC units considered within a given lake's watershed scope (checking 'Yes' in the spreadsheet if included).  
...
* Please visit the HUC to Lake excel spreadsheet. Does the aggregation and selection of HUC6/HUC8/HUC10 within watershed boundary appear correct? Please provided feedback and edits where you see fit.
...
field_meas_sites_in_watershed <- p1_site_in_watersheds_sf %>%
  ...
  leaflet_map_sites <- leaflet() %>% 
    ...
    addLayersControl(baseGroups = c("Labels", "No Labels"),
                     ...
                     position = "topright",
                     options = layersControlOptions(collapsed = F)) %>% 
    hideGroup(c("Gage sites (dv)","Gage sites (field measurements)","Streams (order 3)"))
...
| Huc | [National Hydrological Dataset - high res](https://www.usgs.gov/national-hydrography/nhdplus-high-resolution) - HUC6,8,10,12 | The nhdplusTools R package was used to collect this data
